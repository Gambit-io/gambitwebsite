...
// Map spreadsheet rows (objects keyed by column header, from an .xlsx or .csv)
// into personas. affirms and negates may be separated by semicolons, pipes, or
// newlines within a single cell.
export function parsePersonaRows(rows) {
  if (!Array.isArray(rows) || rows.length === 0) throw new Error('No rows found in the sheet');
  const splitList = (v) => String(v == null ? '' : v).split(/\s*[;|\n]\s*/).map((s) => s.trim()).filter(Boolean);
  const get = (row, ...keys) => {
    for (const k of keys) {
  ...
  return rows.map((row, i) => normalizePersona({
    id: get(row, 'id'),
    display_name: get(row, 'display_name', 'name'),
    age: Number(get(row, 'age')) || undefined,
    sex: get(row, 'sex'),
    complaint: get(row, 'complaint'),
    expected_endpoint: get(row, 'expected_endpoint', 'expected endpoint', 'endpoint'),
    affirms: splitList(get(row, 'affirms')),
    negates: splitList(get(row, 'negates')),
    clinical_truth: get(row, 'clinical_truth', 'clinical truth'),
    character: {
      personality: get(row, 'personality'),
      health_literacy: get(row, 'health_literacy', 'health literacy', 'literacy'),
      emotional_state: get(row, 'emotional_state', 'emotional state', 'state'),
      speech_style: get(row, 'speech_style', 'speech style', 'speech'),
    },
  }, i));
}
...
export async function runBatch({ personas, callClaude, models, concurrency = 3, onRunStart, onRunDone } = {}) {
  ...
  async function worker() {
    while (true) {
      ...
      try {
        results[i] = await runOne({ persona, callClaude, models });
      } catch (err) {
        results[i] = {
