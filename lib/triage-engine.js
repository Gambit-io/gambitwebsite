...
export function computeReport(results) {
  ...
  const fid = results.map((r) => r.fidelity_score).filter((n) => typeof n === 'number');
  const avgFidelity = fid.length ? Math.round((fid.reduce((a, b) => a + b, 0) / fid.length) * 10) / 10 : null;
  ...
  return { total, correct, accuracy, under, over, fail, avgFidelity, byComplaint };
}
